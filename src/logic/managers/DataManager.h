...
public:
    ...
    boost::optional<UID> insertImageRecord( std::shared_ptr<ImageRecord> );

    /// Insert a parcellation record and return its assigned UID.
    boost::optional<UID> insertParcellationRecord( std::shared_ptr<ParcellationRecord> );

    /// Insert a slide record and return its assigned UID.
    boost::optional<UID> insertSlideRecord( std::shared_ptr<SlideRecord> );

    /// Insert an image color map record and return its assigned UID.
    boost::optional<UID> insertImageColorMapRecord( std::shared_ptr<ImageColorMapRecord> );

    /// Insert a label table record and return its assigned UID.
    boost::optional<UID> insertLabelTableRecord( std::shared_ptr<LabelTableRecord> );

    /// Insert an isosurface mesh record associated with the given image
    boost::optional<UID> insertIsoMeshRecord( const UID& imageUid, std::shared_ptr<MeshRecord> );

    /// Insert a label mesh record associated with the given parcellation
    boost::optional<UID> insertLabelMeshRecord( const UID& parcelUid, std::shared_ptr<MeshRecord> );

    /// Insert a reference image landmark group record associated with the given image
    boost::optional<UID> insertRefImageLandmarkGroupRecord(
    ...
    bool setActiveImageUid( const boost::optional<UID>& imageUid );
    bool setActiveParcellationUid( const boost::optional<UID>& parcelUid );
    ...
    boost::optional<UID> activeImageUid() const;

    /// Get parcellation currently being displayed
    boost::optional<UID> activeParcellationUid() const;

    /// Get the active slide UID. This is the slide currently being manipulated by user.
    /// If there are no slides, then boost::none is returned.
    boost::optional<UID> activeSlideUid() const;

    /// Get index of the active slide
    /// If there are no slides, then boost::none is returned.
    boost::optional<size_t> activeSlideIndex() const;

    /// Get index of a slide within the Slide Stack.
    boost::optional<size_t> slideIndex( const UID& slideUid ) const;
    ...
    boost::optional<UID> defaultParcellationUid_of_image( const UID& imageUid ) const;

    /// Get UID of parcellation used to generate a given label mesh
    boost::optional<UID> parcellationUid_of_labelMesh( const UID& labelMeshUid ) const;

    /// Get UID of image used to generate a given isosurface mesh
    boost::optional<UID> imageUid_of_isoMesh( const UID& isoMeshUid ) const;

    /// Get UID of image color map associated with an image
    boost::optional<UID> imageColorMapUid_of_image( const UID& imageUid ) const;

    /// Get UID of label table associated with a parcellation
    boost::optional<UID> labelTableUid_of_parcellation( const UID& parcelUid ) const;

    /// Get UID of the default greyscale image color map
    boost::optional<UID> defaultImageColorMapUid() const;

    /// Get UID of reference image associated with a given group of landmarks
    boost::optional<UID> imageUid_of_landmarkGroup( const UID& lmGroupUid ) const;

    /// Get UID of slide associated with a given group of landmarks
    boost::optional<UID> slideUid_of_landmarkGroup( const UID& lmGroupUid ) const;

    /// Get UID of slide associated with a given annotation
    boost::optional<UID> slideUid_of_annotation( const UID& annotUid ) const;



    /// Get UID of image at given ordered index
    boost::optional<UID> orderedImageUid( long index );

    /// Get UID of parcellation at given ordered index
    boost::optional<UID> orderedParcellationUid( long index );

    /// Get UID of slide at given ordered index
    boost::optional<UID> orderedSlideUid( long index );

    /// Get UID of image color map at given ordered index
    boost::optional<UID> orderedImageColorMapUid( long index );

    /// Get UID of reference image landmark group at given ordered index
    boost::optional<UID> orderedRefImageLandmarkGroupUid( const UID& imageUid, long index );

    /// Get UID of slide landmark group at given ordered index
    boost::optional<UID> orderedSlideLandmarkGroupUid( const UID& slideUid, long index );

    /// Get UID of slide annotation at given ordered index
    boost::optional<UID> orderedSlideAnnotationUid( const UID& slideUid, long index );



    /// Get index of image with given UID
    boost::optional<long> orderedImageIndex( const UID& uid );

    /// Get index of parcellation with given UID
    boost::optional<long> orderedParcellationIndex( const UID& uid );

    /// Get index of slide with given UID
    boost::optional<long> orderedSlideIndex( const UID& uid );

    /// Get index of slide with given UID
    boost::optional<long> orderedImageColorMapIndex( const UID& uid );

    /// Get index of reference image landmark group with given UID
    boost::optional<long> orderedRefImageLandmarkGroupIndex( const UID& imageUid, const UID& lmGroupUid );

    /// Get index of slide landmark group with given UID
    boost::optional<long> orderedSlideLandmarkGroupIndex( const UID& slideUid, const UID& lmGroupUid );

    /// Get index of slide annotation with given UID
    boost::optional<long> orderedSlideAnnotationIndex( const UID& slideUid, const UID& annotUid );
